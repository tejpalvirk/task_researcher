...
from .utils import truncate, read_json, log
...
def format_phase(phase: Optional[int]) -> Text:
    """Formats phase number."""
    if phase is None:
        return Text("N/A", style="dim")
    # Style phase number
    return Text(f"Phase {phase}", style="cyan")
...
def format_dependencies(
    ...
    if not dependencies: return Text("None", style="grey50") if not simple else "None"
    formatted_deps = []
    from .utils import find_task_by_id as find_task_dict
    all_tasks_dicts = [t.model_dump() for t in all_tasks]
...
# --- Display Functions ---
def display_banner():
    console.print(Panel(Text("Task Researcher", style="bold blue", justify="center"), title="[bold cyan]Welcome[/]", border_style="cyan"))
...
def display_tasks_summary(tasks: List[models.Task]):
    ...
    table.add_column("Phase", width=8, justify="center") # Adjusted width
    table.add_column("Title", min_width=30, ratio=3)
    table.add_column("Status", justify="center", width=12)
    table.add_column("Prio", justify="center", width=6)
    ...
    for task in tasks:
        ...
        phase_text = str(task.phase) if task.phase is not None else "[dim]N/A[/]" # Display number or N/A
        table.add_row(
            str(task.id), phase_text, task.title, format_status(task.status),
            format_priority(task.priority), str(subtask_count), str(dep_count),
        )
    console.print(table)
...
def display_subtasks_summary(subtasks: List[models.Subtask], parent_id: int):
    ...
    table.add_column("Status", justify="center", width=12)
    table.add_column("Acceptance Criteria", ratio=3)
    ...
    for subtask in subtasks:
         ...
         table.add_row(f"{parent_id}.{subtask.id}", subtask.title, format_status(subtask.status), acceptance_crit, sibling_deps_str)
    console.print(table)
...
def display_task_details(task: Union[models.Task, models.Subtask], all_tasks: List[models.Task]):
    ...
    if not is_subtask:
         ...
         if parent_info: panel_content.append(f"Parent Task: #{parent_info['id']} ({parent_info['title']})\n", style="italic cyan")

    panel_content.append("\n[bold]Description:[/bold]\n", style="white"); panel_content.append(task.description or "N/A").append("\n")
    panel_content.append("\n[bold]Implementation Details:[/bold]\n", style="white"); panel_content.append(task.details or "N/A").append("\n")

    if is_subtask:
         panel_content.append("\n[bold]Acceptance Criteria:[/bold]\n", style="white"); panel_content.append(task.acceptanceCriteria or "N/A").append("\n")
    elif not is_subtask:
        panel_content.append("\n[bold]Test Strategy:[/bold]\n", style="white"); panel_content.append(task.testStrategy or "N/A").append("\n")

    if not is_subtask and task.subtasks:
        panel_content.append("\n[bold]Subtasks:[/bold]\n", style="white")
        sub_table = Table(show_header=True, box=None, padding=(0, 1), expand=False)
        sub_table.add_column("ID", style="dim", no_wrap=True); sub_table.add_column("Title", ratio=2); sub_table.add_column("Status", justify="center"); sub_table.add_column("AC", justify="left", ratio=1); sub_table.add_column("Deps", justify="center")
        for sub in task.subtasks:
            ...
            sub_table.add_row(f"{task.id}.{sub.id}", sub.title, format_status(sub.status), ac_short, Text(sibling_deps_text, style="cyan"))
...
def format_task_for_file(task: models.Task, all_tasks: List[models.Task]) -> str:
    ...
    content += f"# Phase: {task.phase if task.phase is not None else 'N/A'}\n" # Added phase number
...
def display_complexity_report(report_path: Path):
    ...
    if not report_data: console.print(f"[bold red]Error:[/bold red] Complexity report not found at {report_path}"); return
    try: report = models.ComplexityReport.model_validate(report_data)
    except Exception as e: log.error(f"Invalid complexity report structure: {e}"); console.print(f"[bold red]Error:[/bold red] Invalid complexity report file structure."); return
    meta = report.meta
    ...
    table.add_column("ID", style="dim", width=7, no_wrap=True); table.add_column("Title", min_width=30, ratio=2); table.add_column("Complexity", justify="center", width=12); table.add_column("Rec. Sub", justify="center", width=10); table.add_column("Reasoning / Expand Prompt", ratio=3)
    ...
    for item in sorted_analysis:
        ...
        reasoning_prompt = Text(); reasoning_prompt.append(item.reasoning or "N/A", style="italic"); reasoning_prompt.append("\nExpand Prompt: ", style="dim"); reasoning_prompt.append(truncate(item.expansionPrompt, 150))
        row_style = "on grey19" if item.complexityScore >= meta.get('thresholdScore', 5.0) else ""
        table.add_row(str(item.taskId), item.taskTitle, complexity_text, str(item.recommendedSubtasks), reasoning_prompt, style=row_style)
    console.print(table)
    scores = [item.complexityScore for item in report.complexityAnalysis]
    if scores:
         avg_score = sum(scores) / len(scores); high_count = sum(1 for s in scores if s >= 8); med_count = sum(1 for s in scores if 4 <= s < 8); low_count = sum(1 for s in scores if s < 4)
         console.print("\n[bold]Summary Stats:[/bold]"); console.print(f"  Average Complexity: {avg_score:.1f}/10"); console.print(f"  Distribution: [red]High ({high_count})[/], [yellow]Medium ({med_count})[/], [green]Low ({low_count})[/]")


def display_complexity_summary(report: models.ComplexityReport):
     scores = [item.complexityScore for item in report.complexityAnalysis];
     if not scores: console.print("[yellow]No complexity scores generated.[/yellow]"); return
     avg_score = sum(scores) / len(scores); high_count = sum(1 for s in scores if s >= 8); med_count = sum(1 for s in scores if 4 <= s < 8); low_count = sum(1 for s in scores if s < 4); threshold = report.meta.get('thresholdScore', 5.0); recommended_for_expansion = sum(1 for item in report.complexityAnalysis if item.complexityScore >= threshold)
     console.print(Panel(f"Analyzed [bold]{len(scores)}[/] tasks. Average Complexity: [bold]{avg_score:.1f}/10[/].\nDistribution: [red]High ({high_count})[/] | [yellow]Medium ({med_count})[/] | [green]Low ({low_count})[/].\n[bold]{recommended_for_expansion}[/] tasks recommended for expansion (score >= {threshold}).", title="Complexity Summary", border_style="green", expand=False))
     report_file_arg = f"--report-file={report.meta.get('inputFile', config.COMPLEXITY_REPORT_PATH)}"
     console.print(f"View full details: [cyan]task-researcher complexity-report {report_file_arg}[/]")

# --- STORM UI --- (Added research plan display)
